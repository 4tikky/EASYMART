...
{{-- 2. Judul Halaman --}}
@section('title', 'Detail Penjual - ' . $seller->nama_toko)
...
@section('content')
    ...
    <div class="w-full max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            ...
            {{-- Badge Status --}}
            @php
                $statusColor = match($seller->status_verifikasi) {
                    \App\Models\User::STATUS_ACTIVE => 'bg-emerald-100 text-emerald-800 border-emerald-200',
                    \App\Models\User::STATUS_PENDING => 'bg-amber-100 text-amber-800 border-amber-200',
                    \App\Models\User::STATUS_REJECTED => 'bg-rose-100 text-rose-800 border-rose-200',
                    default => 'bg-slate-100 text-slate-800 border-slate-200'
                };
            @endphp
            <span class="px-4 py-2 rounded-full text-sm font-bold border {{ $statusColor }} shadow-sm flex items-center gap-2">
                Status: {{ strtoupper($seller->status_verifikasi) }}
            </span>
        </div>
            ...
            <section class="card-section h-full">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <p class="info-value">{{ $seller->name }}</p>
                    </div>
                    <div>
                        <p class="info-label">Email Login</p>
                        <p class="info-value">{{ $seller->email }}</p>
                    </div>
                    <div>
                        <p class="info-label">Nomor Handphone</p>
                        <p class="info-value flex items-center gap-2">
                            {{ $seller->no_handphone_pic }}
                            <a href="https://wa.me/{{ preg_replace('/^0/', '62', $seller->no_handphone_pic) }}" target="_blank" class="text-xs bg-emerald-50 text-emerald-600 border border-emerald-200 px-2 py-0.5 rounded hover:bg-emerald-100 transition">
                                Chat WA ↗
                            </a>
                        </p>
                    </div>
                    <div>
                        ...
                        <p class="info-value">{{ $seller->no_ktp_pic }}</p>
                    </div>
                </div>
            </section>
            ...
            <section class="card-section h-full">
                ...
                <div class="grid grid-cols-1 gap-y-5">
                    <div>
                        ...
                        <p class="info-value text-lg text-emerald-800">{{ $seller->nama_toko }}</p>
                    </div>
                    <div>
                        <p class="info-label">Deskripsi Toko</p>
                        <p class="text-slate-600 italic text-sm">{{ $seller->deskripsi_singkat ?: 'Tidak ada deskripsi.' }}</p>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            ...
                            <p class="info-value">{{ $seller->alamat_pic }}</p>
                        </div>
                        <div>
                            <p class="info-label">RT / RW</p>
                            <p class="info-value">{{ $seller->rt }} / {{ $seller->rw }}</p>
                        </div>
                        <div>
                            <p class="info-label">Kelurahan</p>
                            <p class="info-value">{{ $seller->nama_kelurahan }}</p>
                        </div>
                        <div>
                            <p class="info-label">Kota/Kab</p>
                            <p class="info-value">{{ $seller->kabupaten_kota }}</p>
                        </div>
                        <div>
                            <p class="info-label">Provinsi</p>
                            <p class="info-value">{{ $seller->provinsi }}</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-3">
                    ...
                    @if($seller->file_upload_ktp_pic)
                        <div class="relative group bg-slate-50 border border-slate-200 rounded-xl overflow-hidden shadow-sm h-64 flex items-center justify-center">
                            <img src="{{ asset('storage/'.$seller->file_upload_ktp_pic) }}" 
                                 class="max-w-full max-h-full object-contain"
                                 alt="Scan KTP">
                            <a href="{{ asset('storage/'.$seller->file_upload_ktp_pic) }}" target="_blank" 
                               class="absolute inset-0 bg-black/50 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition font-medium">
                                <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"/></svg>
                                Perbesar KTP
                            </a>
